...
static vgui::DHANDLE<CBFModCreditsDialog> g_pBFModCreditsDialog;
static vgui::DHANDLE<CFTModCreditsDialog> g_pFTModCreditsDialog;
...
void CL_OpenBFModCreditsDialog(const CCommand& args)
{
	if (g_pBFModCreditsDialog.Get() == NULL)
	{
		g_pBFModCreditsDialog = vgui::SETUP_PANEL(new CBFModCreditsDialog(NULL));
	}

	g_pBFModCreditsDialog->Deploy();
}
void CL_OpenFTModCreditsDialog(const CCommand& args)
{
	if (g_pFTModCreditsDialog.Get() == NULL)
	{
		g_pFTModCreditsDialog = vgui::SETUP_PANEL(new CFTModCreditsDialog(NULL));
	}

	g_pFTModCreditsDialog->Deploy();
}
...
static ConCommand openmodcredits( "openmodcredits", &CL_OpenBFModCreditsDialog, "Displays the TF2 Advanced Options dialog." );
static ConCommand tfft_credits( "tfft_credits", &CL_OpenFTModCreditsDialog, "Displays the TF2 Advanced Options dialog." );
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CBFModCreditsDialog::CBFModCreditsDialog(vgui::Panel* parent) : BaseClass(NULL, "BFModCreditsDialog")
{
	// Need to use the clientscheme (we're not parented to a clientscheme'd panel)
	vgui::HScheme scheme = vgui::scheme()->LoadSchemeFromFileEx(enginevgui->GetPanel(PANEL_CLIENTDLL), "resource/ClientScheme.res", "ClientScheme");
	SetScheme(scheme);
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
CBFModCreditsDialog::~CBFModCreditsDialog()
{
	delete m_pDescription;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBFModCreditsDialog::ApplySchemeSettings(vgui::IScheme* pScheme)
{
	BaseClass::ApplySchemeSettings(pScheme);

	LoadControlSettings("resource/ui/BFModCreditsDialog.res");
	m_pListPanel->SetFirstColumnWidth(0);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBFModCreditsDialog::ApplySettings(KeyValues* inResourceData)
{
	BaseClass::ApplySettings(inResourceData);
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBFModCreditsDialog::OnClose()
{
	BaseClass::OnClose();
...
// Input  : *command - 
//-----------------------------------------------------------------------------
void CBFModCreditsDialog::OnCommand(const char* command)
{
...
void CBFModCreditsDialog::OnKeyCodeTyped(KeyCode code)
{
	// force ourselves to be closed if the escape key it pressed
	if (code == KEY_ESCAPE)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBFModCreditsDialog::OnKeyCodePressed(KeyCode code)
{
	// force ourselves to be closed if the escape key it pressed
	if (GetBaseButtonCode(code) == KEY_XBUTTON_B || GetBaseButtonCode(code) == STEAMCONTROLLER_B || GetBaseButtonCode(code) == STEAMCONTROLLER_START)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBFModCreditsDialog::GatherCurrentValues()
{
	if (!m_pDescription)
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBFModCreditsDialog::CreateControls()
{
	DestroyControls();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBFModCreditsDialog::DestroyControls()
{
	mpcontrol_t* p, * n;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBFModCreditsDialog::Deploy(void)
{
	SetVisible(true);
	MakePopup();
	MoveToFront();
...
#define TFFT_CREDITS_FILE "cfg/tfft_credits.txt"

//-----------------------------------------------------------------------------
// Purpose: Constructor
//-----------------------------------------------------------------------------
CFTModCreditsDialog::CFTModCreditsDialog(vgui::Panel* parent) : BaseClass(NULL, "BFModCreditsDialog")
{
	// Need to use the clientscheme (we're not parented to a clientscheme'd panel)
	vgui::HScheme scheme = vgui::scheme()->LoadSchemeFromFileEx(enginevgui->GetPanel(PANEL_CLIENTDLL), "resource/ClientScheme.res", "ClientScheme");
	SetScheme(scheme);
	...
	m_pDescription = new CInfoDescription();
	m_pDescription->InitFromFile(TFFT_CREDITS_FILE);
	m_pDescription->TransferCurrentValues(NULL);
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
CFTModCreditsDialog::~CFTModCreditsDialog()
{
	delete m_pDescription;
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CFTModCreditsDialog::ApplySchemeSettings(vgui::IScheme* pScheme)
{
	BaseClass::ApplySchemeSettings(pScheme);

	LoadControlSettings("resource/ui/FTModCreditsDialog.res");
	m_pListPanel->SetFirstColumnWidth(0);
...
// Purpose:
//-----------------------------------------------------------------------------
void CFTModCreditsDialog::ApplySettings(KeyValues* inResourceData)
{
	BaseClass::ApplySettings(inResourceData);
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CFTModCreditsDialog::OnClose()
{
	BaseClass::OnClose();
...
// Input  : *command -
//-----------------------------------------------------------------------------
void CFTModCreditsDialog::OnCommand(const char* command)
{
	if (!stricmp(command, "Ok"))
	{
		OnClose();
	...
	else if (!stricmp(command, "BF2Credits"))
	{
		engine->ClientCmd("openmodcredits");
	}

	BaseClass::OnCommand(command);
}

void CFTModCreditsDialog::OnKeyCodeTyped(KeyCode code)
{
	// force ourselves to be closed if the escape key it pressed
	if (code == KEY_ESCAPE)
	{
...
// Purpose:
//-----------------------------------------------------------------------------
void CFTModCreditsDialog::OnKeyCodePressed(KeyCode code)
{
	// force ourselves to be closed if the escape key it pressed
	if (GetBaseButtonCode(code) == KEY_XBUTTON_B || GetBaseButtonCode(code) == STEAMCONTROLLER_B || GetBaseButtonCode(code) == STEAMCONTROLLER_START)
	{
...
// Purpose:
//-----------------------------------------------------------------------------
void CFTModCreditsDialog::GatherCurrentValues()
{
	if (!m_pDescription)
		return;
...
// Purpose:
//-----------------------------------------------------------------------------
void CFTModCreditsDialog::CreateControls()
{
	DestroyControls();
...
// Purpose:
//-----------------------------------------------------------------------------
void CFTModCreditsDialog::DestroyControls()
{
	mpcontrol_t* p, * n;
...
// Purpose:
//-----------------------------------------------------------------------------
void CFTModCreditsDialog::Deploy(void)
{
	SetVisible(true);
	MakePopup();
	MoveToFront();
